...
for i in range(1, 24):  # Assuming you have files named from 1 to 23
    ...
    if os.path.exists(All_Files_Directory):
        ...
        if(len(filtered_columns)>0):
            ...
            balanced_set = pd.concat([train[train["Y"]==0].sample(len(balanced_set)*10), balanced_set],ignore_index=True)
...
boruta = BorutaPy(rf, n_estimators='auto',verbose=2, max_iter=10)#, random_state=42)
 
balanced_set = combined_df[combined_df["Y"]>0.0].copy()
balanced_set = pd.concat([combined_df[combined_df["Y"]==0].sample(len(balanced_set)*100), balanced_set],ignore_index=True)
balanced_set.replace([np.nan], 0, inplace=True)
feature_drop = balanced_set.filter(OPTIONAL_FEATURE_OMISSION)
balanced_set.drop(feature_drop, axis=1, inplace = True)
boruta.fit(balanced_set.drop(FEATURE_OMISSION, axis=1).values, balanced_set["Y"].values)
cols = balanced_set.drop(FEATURE_OMISSION, axis=1, inplace = False).columns
